...
trap "gcloud dns record-sets transaction abort -z $gce_friendly_name" ERR
gcloud dns record-sets transaction start -z "$gce_friendly_name"
...
if [ -n "$record_line" ] ; then
  gcloud dns record-sets transaction remove -z $gce_friendly_name \
...
gcloud dns record-sets transaction add -z $gce_friendly_name \
...
gcloud dns record-sets transaction execute -z $gce_friendly_name
