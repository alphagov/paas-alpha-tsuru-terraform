...
resource "google_compute_address" "tsuru-sslproxy" {
  ...
  provisioner "local-exec" {
    command = "./ensure_gce_dns.sh ${var.dns_zone_id} proxy.tsuru2.paas.alphagov.co.uk 60 A ${google_compute_address.tsuru-sslproxy.address}"
  }
  provisioner "local-exec" {
    command = "./ensure_gce_dns.sh ${var.dns_zone_id} *.hipache.tsuru2.paas.alphagov.co.uk 60 CNAME proxy.tsuru2.paas.alphagov.co.uk."
  }
}
